...
import { deleteCarts, getCartByUserId, updateCart } from '@/api/cart' 
...
const confirmCartOrder = async () => {
  if (!selectedAddress.value) {
    ...
    const orderData = {
      ...
      orderItems: cartItems.value
       ...
       .map(item => ({
          ...
          price: parseFloat((item.totalPrice / item.quantity).toFixed(2))
        }))
    }
    ...
    if (res.code === 1) {
      ...
      //更新全局购物车数量
      await cartStore.fetchCartCount() // 确保数量同步
    } else {
