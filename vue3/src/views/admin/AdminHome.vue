...
const loadAdminInfo = async () => {
  try {
    ...
    userInfo.value = {
      id: res.data.id, // 确保API返回ID
      name: res.data.name || '管理员',
      avatar: res.data.avatar || userInfo.value.avatar
    }
    localStorage.setItem('currentAdminId', res.data.id) // 保存当前登录ID
  } catch (error) {
...
// 在组件挂载时监听管理员更新事件
onMounted(() => {
  eventBus.on('admin-updated', loadAdminInfo)
  loadAdminInfo()
})

// 组件卸载前移除监听
onBeforeUnmount(() => {
  eventBus.off('admin-updated', loadAdminInfo)
})
// 处理退出登录事件
const handleLogout = () => {
  console.log('退出登录')
        ...
        <el-container>
            ...
            <el-container style="margin-top: 60px;"> 
                ...
                <el-aside width="200px" class="aside">
                    ...
                    <el-menu router>
                        <!-- 首页菜单 -->
                        <el-menu-item index="/admin/Adminindex">
                        ...
                        <el-menu-item index="/admin/Admincarousel">
                            <el-icon><Refresh /></el-icon>轮播图管理
                        </el-menu-item>
                         <el-menu-item index="/admin/Adminlog">
                            <el-icon><Edit /></el-icon>日志管理
                        </el-menu-item>
